/*
 * 
 * modified 2014-02-03 ebeslagic@voanews.com
 * 
 * added url_suffix to deal with caching / breaking out of browser cache
 * 
 */
(function(global) {
  "use strict";
  ...
  */
  ...
  var Tabletop = function(options) {
    ...
    this.url_suffix = options.url_suffix;
      ...
      if(this.simple_url) {
        ...
        if(path.indexOf("/list/") !== -1) {
          // script.src = this.endpoint + "/" + this.key + "-" + path.split("/")[4];
        	script_src = this.endpoint + "/" + this.key + "-" + path.split("/")[4];
        } else {
          // script.src = this.endpoint + "/" + this.key;
        	script_src = this.endpoint + "/" + this.key;
        }
      } else {
        //script.src = this.endpoint + url;
    	  script_src = this.endpoint + url;
      }
      
      if (this.parameterize) {
        // script.src = this.parameterize + encodeURIComponent(script.src);
        script_src = this.parameterize + encodeURIComponent(script.src);
      }
      
      // used for cache busting
      if( typeof this.url_suffix != 'undefined' ) {
    	  script_src += this.url_suffix;
      }
